...
AI가 등급을 확정하지 않지만, 사용자가 스스로 판단할 수 있도록 다음 세 가지를
함께 표시한다.

**① AI 추정 등급을 기본값으로 제안**

AI가 추정한 등급(`CommonReviewItem.grade`)을 라디오 버튼의 기본 선택값으로
설정한다. 사용자는 동의하면 [저장]만 누르면 되고, 다르면 변경 후 저장한다.
판단 부담을 최소화하면서도 최종 결정권은 사용자에게 있다.

**② 등급 판단 가이드 표시**

회사 가이드라인(C/S/O 등급 체계)의 핵심을 사용자가 선택하는 순간 볼 수 있도록
제공한다. 구현 방식은 PHP 팀과 협의하되 다음 세 단계로 점진 적용한다.

```text
1단계 (빠른 구현): Tooltip
  - HTML title 속성으로 마우스 오버 시 설명 표시
  - PHP 10줄 이내 구현 가능

2단계 (개선): 사이드 패널
  - 화면 오른쪽에 항상 열려있거나 [?] 버튼 클릭 시 표시
  - 전체 review 화면에 공통 적용

3단계 (심화): 현재 문장 기준 강조
  - sensitiveCategory 필드를 활용해 해당 등급만 강조
  - AI가 왜 이 등급을 추정했는지 사용자가 즉시 이해 가능
```

**③ 탐지 이유 표시**

`reason` 문자열 또는 `sensitiveCategory` 필드를 활용해 AI가 어떤 이유로
이 문장을 추천했는지 표시한다.
...
├──────────────────────────────┼──────────────────────────────┤
│ 3쪽 5번째 줄                 │ C급 (기밀)                   │
│ "입찰 평가표를 검토했습니다. │   외부 AI 사용 불가          │
│  향후 협상 전략은..."         │   · 시스템 계정/비밀번호     │
│                              │   · IP/VLAN/네트워크 구성도  │
│ AI 추정: S급 (확신도 75%)    │   · 중요시설 위치/도면       │
│ 감지 이유: 입찰·계약·협상    ├──────────────────────────────┤
│                              │ S급 (민감) ← 이 케이스 해당 │
│ ⚪ C급 (기밀)                │   비식별화 후 사용 가능      │
│ 🔵 S급 (민감) (AI 추정)      │   · 성명, 연락처, 이메일     │
│ ⚪ O급 (공개)                │   · 입찰·계약·인사 관련 정보 │
│ ⚪ 잘못된 탐지               ├──────────────────────────────┤
│                              │ O급 (공개)                   │
│ [저장]                       │   그대로 사용 가능           │
│                              │   · C/S 외 모든 정보         │
...
```php
<?php foreach ($result['reviewTargets'] as $idx => $item): ?>
<div class="review-item">
  <p><?= $item['locationLabel'] ?></p>
  <p>"<?= $item['context'] ?>"</p>
  <p>AI 추정: <?= $item['grade'] ?>급 / 감지: <?= $item['sensitiveCategory'] ?></p>

  <?php
  $options = [
      'C' => ['label' => 'C급 (기밀)', 'tip' => '외부 AI 사용 불가. 시스템 계정/IP/중요시설 정보 등'],
      'S' => ['label' => 'S급 (민감)', 'tip' => '비식별화 후 사용 가능. 개인정보·입찰·계약 정보 등'],
      'O' => ['label' => 'O급 (공개)', 'tip' => '그대로 사용 가능. C/S 외 정보 또는 비식별화 완료'],
      'X' => ['label' => '잘못된 탐지', 'tip' => '민감 정보가 없는 오탐지'],
  ];
  foreach ($options as $grade => $opt):
      $checked = ($grade === $item['grade']) ? 'checked' : '';
  ?>
    <label title="<?= $opt['tip'] ?>">
      <input type="radio" name="grade[<?= $idx ?>]"
             value="<?= $grade ?>" <?= $checked ?>>
      <?= $opt['label'] ?>
      <?php if ($grade === $item['grade']): ?>
        <span class="ai-badge">(AI 추정)</span>
      <?php endif; ?>
    </label>
  <?php endforeach; ?>

  <button type="button" onclick="saveFeedback(<?= $idx ?>)">저장</button>
</div>
<?php endforeach; ?>
```
...
PHP 팀이 reviewTargets 표시 시 활용하는 필드:

| 필드 | 용도 |
|---|---|
| `grade` | 라디오 버튼 기본 선택값, 사이드 패널 강조 |
| `sensitiveCategory` | 탐지 이유 표시 ("입찰·계약 관련" 등) |
| `source` | "AI 추정" 뱃지 표시 여부 (`ai`일 때만) |
| `reason` | 상세 이유 (개발자/관리자용 디버그) |
| `context` | 문장 원문 표시 |
| `locationLabel` | 위치 표시 |
