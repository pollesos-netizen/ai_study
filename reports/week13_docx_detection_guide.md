...
src/docx_detector.py           (신규 - docx 탐지 + guide 생성)
...
```text
paragraphNo는 doc.paragraphs (본문) 기준으로만 인덱싱한다.
표/헤더/푸터/각주는 13주차 범위 외이며, section="body"가 아닌 target은 skip + warning 처리한다.
```
...
location_meta = {
    "fileType": "docx",
    "section": "body",      # 13주차에서는 "body"만
    "paragraphNo": para_index,
}
```

14주차 이후 표/헤더/푸터 확장 시 `section="table_cell"`, `"header"`, `"footer"` 등으로 자연스럽게 확장할 수 있다.
...
`notebooks/15_test_docx_detector.py`에서 18개 테스트 케이스를 검증했다. **52/52 모두 통과**했다.
...
| TC17 | section이 body가 아닌 target | PASS |
...
```text
실제 docx 파일
→ 본문 paragraph 순회 (빈 문단 strip 기준 제외)
...
2. 같은 paragraph에 IP + VLAN 두 target이 있어도 정상 처리
3. deletion_mode=mark로 "(삭제됨)" 표시 동작
4. force-mock-review로 reviewTargets 흐름 실증 완료
5. 원본 파일은 변경되지 않고 outputFilePath=None
```
...
6. docx_detector.py 구현 (paragraph 순회, regex/NER/AI 어댑터)
...
2~7번은 향후 detector 확장 단계에서 추가한다.
