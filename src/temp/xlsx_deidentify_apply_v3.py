...
def make_output_path(input_path: str | Path, output_path: str | Path | None = None) -> str:
    ...
    suffix 탐색은 무한 루프 방지를 위해 1000번까지만 수행합니다.
    """
...
def sort_targets_for_display(targets: list[DeidentifyTarget]) -> list[DeidentifyTarget]:
    """
    사용자 표시용 target 순서입니다.

    같은 셀 안에서는 start 오름차순으로 정렬해 셀 안의 자연스러운 읽기 순서와 맞춥니다.
    """
    return sorted(
        targets,
        key=lambda target: (
