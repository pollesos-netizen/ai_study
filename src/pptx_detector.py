...
class ParsedPptxParagraph:
    ...
    - "table_cell":         일반 표 셀
    - "notes":              발표자 노트
    - "group_shape_text":   그룹 shape 내부의 텍스트 shape
    - "group_table_cell":   그룹 shape 내부의 표 셀

    paragraphNo 의미 (section별):
    - shape_text / group_shape_text:  shape.text_frame.paragraphs 인덱스
    - table_cell / group_table_cell:  cell.text_frame.paragraphs 인덱스
    - notes:                           notes_text_frame.paragraphs 인덱스

    필드 사용 여부:
    - shape_no:        shape_text, table_cell, group_*에서 사용
                       (그룹 내부는 child shape 번호)
    - group_shape_no:  group_* section에서만 사용 (부모 group의 shape 번호)
    - row_no, col_no:  table_cell, group_table_cell에서만 사용
    - table_index:     table_cell, group_table_cell에서만 사용
                       (슬라이드 내 표 순번, 0-based 저장, 1-based 표시)

    그룹 내부 위치는 항상 부모 group_shape_no + child shape_no로 구분합니다.
    그룹 안의 그룹은 14주차 PoC 범위 외입니다.
    """
    ...
    text: str
    paragraph_no: int
    shape_no: int | None = None
    row_no: int | None = None
    col_no: int | None = None
    group_shape_no: int | None = None
    ...
    def location_label(self) -> str:
        ...
        형식:
        - shape_text:        "1번 슬라이드: context..."
        - group_shape_text:  "1번 슬라이드 (그룹 내부): context..."
        - table_cell:        "1번 슬라이드 표 N번 R행 C열: context..."
        - group_table_cell:  "1번 슬라이드 (그룹 내부) 표 N번 R행 C열: context..."
        - notes:             "1번 슬라이드 발표자 노트: context..."

        shapeNo는 사용자가 PowerPoint에서 찾을 방법이 없어 노출하지 않습니다.
        대신 표는 슬라이드 내 표 순번(table_index)을 1-based로 표시합니다.
        """
        slide_idx_1based = self.slide_no + 1

        is_group = self.section.startswith("group_")
        group_suffix = " (그룹 내부)" if is_group else ""

        if self.section in ("shape_text", "group_shape_text"):
            base = f"{slide_idx_1based}번 슬라이드{group_suffix}"
        elif self.section in ("table_cell", "group_table_cell"):
            table_disp = (
                f"{self.table_index + 1}번"
                if self.table_index is not None else "?"
            )
            row_disp = (self.row_no + 1) if self.row_no is not None else "?"
            col_disp = (self.col_no + 1) if self.col_no is not None else "?"
            base = (
                f"{slide_idx_1based}번 슬라이드{group_suffix} "
                f"표 {table_disp} {row_disp}행 {col_disp}열"
            )
    ...
    def location_meta(self) -> dict[str, Any]:
        ...
        if self.group_shape_no is not None:
            meta["groupShapeNo"] = self.group_shape_no
...
def _iter_shape_text_paragraphs(
    ...
    *,
    section: str = "shape_text",
    group_shape_no: int | None = None,
) -> Iterator[ParsedPptxParagraph]:
    """
    텍스트 shape의 paragraph를 ParsedPptxParagraph로 yield합니다.

    그룹 내부 호출 시 section="group_shape_text", group_shape_no=부모 group의 shape 번호를 넘깁니다.
    """
...
def _iter_table_cell_paragraphs(
    ...
    *,
    ...
    table_index는 슬라이드 내 표 순번(0-based)입니다.
    """
...
def _iter_shape_recursive(
    shape, slide_no: int, group_shape_no: int,
    *,
    ...
    그룹 내부 child shape에는 child_index를 shape_no로 부여합니다.
    그룹 자신의 shape 번호는 group_shape_no로 보존하므로, 슬라이드 전체 shape 번호와 충돌하지 않습니다.

    그룹 안의 또 다른 그룹은 14주차 PoC 범위 외이므로 무시합니다.

    table_index_counter는 슬라이드 단위로 공유되는 카운터입니다.
    그룹 내부의 표도 슬라이드 전체에서 N번째 표가 됩니다.
    """
    ...
    for child_index, child in enumerate(shape.shapes):
        ...
        if child.has_table:
            table_idx = table_index_counter[0]
            table_index_counter[0] += 1
            yield from _iter_table_cell_paragraphs(
                child, slide_no, child_index,
                section="group_table_cell",
                group_shape_no=group_shape_no,
                table_index=table_idx,
            )
        elif child.has_text_frame:
            yield from _iter_shape_text_paragraphs(
                child, slide_no, child_index,
...
def iter_pptx_paragraphs(prs) -> list[ParsedPptxParagraph]:
    ...
    표 순번(table_index)은 슬라이드 단위로 0-based 부여됩니다.
    그룹 내부의 표도 같은 카운터를 공유하므로 슬라이드 전체에서 일관됩니다.
    """
    results: list[ParsedPptxParagraph] = []

    for slide_index, slide in enumerate(prs.slides):
        # 슬라이드 단위 표 순번 카운터 (list로 감싸 mutable 공유)
        table_index_counter: list[int] = [0]

        for shape_index, shape in enumerate(slide.shapes):
            if _is_group_shape(shape):
                results.extend(
                    _iter_shape_recursive(
                        shape, slide_index, shape_index,
                        table_index_counter=table_index_counter,
                    )
                )
                continue
...
def _make_pptx_location_key(meta: dict[str, Any]) -> tuple | None:
    ...
    - group_shape_text:  (slideNo, "group_shape_text", groupShapeNo, shapeNo, paragraphNo)
    - group_table_cell:  (slideNo, "group_table_cell", groupShapeNo, shapeNo, rowNo, colNo, paragraphNo)

    그룹 내부 위치는 groupShapeNo + 내부 child shapeNo로 구분합니다.
    """
    ...
    if section == "group_shape_text":
        group_shape_no = meta.get("groupShapeNo")
        shape_no = meta.get("shapeNo")
        if group_shape_no is None or shape_no is None:
            return None
        return (
            int(slide_no), "group_shape_text",
            int(group_shape_no), int(shape_no), int(paragraph_no),
        )

    if section == "group_table_cell":
        group_shape_no = meta.get("groupShapeNo")
        shape_no = meta.get("shapeNo")
        row_no = meta.get("rowNo")
        col_no = meta.get("colNo")
        if (group_shape_no is None or shape_no is None
                or row_no is None or col_no is None):
            return None
        return (
            int(slide_no), "group_table_cell",
            int(group_shape_no), int(shape_no),
            int(row_no), int(col_no), int(paragraph_no),
        )
...
def _format_location_meta_for_label(meta: dict[str, Any]) -> str:
    ...
    is_group = isinstance(section, str) and section.startswith("group_")
    group_suffix = " (그룹 내부)" if is_group else ""

    if section in ("shape_text", "group_shape_text"):
        return f"{slide_disp}{group_suffix}"
    if section in ("table_cell", "group_table_cell"):
        table_idx = meta.get("tableIndex")
        row = meta.get("rowNo")
        col = meta.get("colNo")
        table_disp = f"{table_idx + 1}번" if table_idx is not None else "?"
        row_disp = (row + 1) if row is not None else "?"
        col_disp = (col + 1) if col is not None else "?"
        return f"{slide_disp}{group_suffix} 표 {table_disp} {row_disp}행 {col_disp}열"
    if section == "notes":
...
def _group_targets_by_location(
    ...
    for target in targets:
        ...
        if key is None:
            ...
            if section not in {
                "shape_text", "table_cell", "notes",
                "group_shape_text", "group_table_cell",
            }:
