...
CONFUSION_MATRIX_PATH = REPORT_DIR / "week8_tensorflow_confusion_matrix.csv"
GRADE_REPORT_PATH = REPORT_DIR / "week8_tensorflow_grade_report.csv"
...
def analyze_validation_results(
    ...
    grade_report_path: Path = GRADE_REPORT_PATH,
) -> None:
    """
    검증 데이터 전체에 대한 등급별 분석 결과를 출력하고 CSV로 저장합니다.
    ...
    # confusion[true_grade][pred_grade] = count
    confusion: dict[str, dict[str, int]] = {
        true_grade: {pred_grade: 0 for pred_grade in grades}
    ...
    print("-" * 32)
    for true_grade in grades:
        values = [str(confusion[true_grade][pred_grade]) for pred_grade in grades]
        print(f"{true_grade:^9} | " + " | ".join(f"{value:^3}" for value in values))

    grade_report_rows: list[dict[str, str]] = []

    print("\n등급별 재현율")
    for grade in grades:
        total = sum(confusion[grade].values())
        correct = confusion[grade][grade]
        recall = correct / total if total else 0.0

        print(f"- {grade}: {correct}/{total} = {recall:.4f}")

        grade_report_rows.append(
            {
                "grade": grade,
                "true_count": str(total),
                "correct_count": str(correct),
                "recall": f"{recall:.6f}",
            }
        )

    pred_counter = Counter(ID_TO_GRADE[int(pred)] for pred in preds)
    ...
    with confusion_matrix_path.open("w", encoding="utf-8-sig", newline="") as f:
        fieldnames = ["true_grade", "pred_C", "pred_S", "pred_O"]
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
            ...
            writer.writerow(
                {
                    ...
                    "pred_C": confusion[true_grade]["C"],
                    "pred_S": confusion[true_grade]["S"],
                    "pred_O": confusion[true_grade]["O"],
                }
            )

    with grade_report_path.open("w", encoding="utf-8-sig", newline="") as f:
        fieldnames = ["grade", "true_count", "correct_count", "recall"]
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(grade_report_rows)

    print("\n=== 분석 CSV 저장 완료 ===")
    print(confusion_matrix_path)
    print(grade_report_path)
