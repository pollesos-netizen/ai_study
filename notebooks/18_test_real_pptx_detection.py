"""
실제 pptx 파일 대상 regex + NER + AI 탐지 + guide 생성 통합 테스트

실행 예시:
    python notebooks/18_test_real_pptx_detection.py "data/test.pptx"
...
설명:
- 실제 pptx의 모든 paragraph(본문 shape / 표 셀 / 발표자 노트 / 그룹 내부)를 순회합니다.
...
- 시스템은 pptx 파일을 직접 수정하지 않습니다 (guide 모드).
"""
...
def inject_mock_review_target(plan: DeidentifyPlan, pptx_path: str) -> DeidentifyPlan:
    """
    첫 번째 paragraph에 mock review target을 강제로 추가합니다.
...
def main() -> None:
    ...
    parser.add_argument("input_path", help="실제 pptx 파일 경로")
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="출력할 autoResults/reviewTargets의 최대 개수",
    )
    ...
    # 사전 정보: 슬라이드/paragraph 수
    prs = load_pptx(str(input_path))
